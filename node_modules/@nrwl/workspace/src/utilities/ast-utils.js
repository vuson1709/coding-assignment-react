...
function insertImport(host, source, fileToEdit, symbolName, fileName, isDefault = false) {
    ...
    const allImports = (0, typescript_2.findNodes)(rootNode, ts.SyntaxKind.ImportDeclaration);
    ...
    if (relevantImports.length > 0) {
        ...
        relevantImports.forEach((n) => {
            Array.prototype.push.apply(imports, (0, typescript_2.findNodes)(n, ts.SyntaxKind.Identifier));
            if ((0, typescript_2.findNodes)(n, ts.SyntaxKind.AsteriskToken).length > 0) {
        ...
        if (importTextNodes.length === 0) {
            const fallbackPos = (0, typescript_2.findNodes)(relevantImports[0], ts.SyntaxKind.CloseBraceToken)[0].getStart() ||
                (0, typescript_2.findNodes)(relevantImports[0], ts.SyntaxKind.FromKeyword)[0].getStart();
            return insertAfterLastOccurrence(host, source, imports, `, ${symbolName}`, fileToEdit, fallbackPos);
        }
        return source;
    }
    // no such import declaration exists
    const useStrict = (0, typescript_2.findNodes)(rootNode, ts.SyntaxKind.StringLiteral).filter((n) => n.text === 'use strict');
    let fallbackPos = 0;
...
function addGlobal(host, source, modulePath, statement) {
    const allImports = (0, typescript_2.findNodes)(source, ts.SyntaxKind.ImportDeclaration);
