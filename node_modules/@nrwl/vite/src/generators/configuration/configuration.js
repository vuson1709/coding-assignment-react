...
async function viteConfigurationGenerator(tree, schema) {
    const tasks = [];
    const { targets , projectType  } = (0, _devkit.readProjectConfiguration)(tree, schema.project);
   ...
   *  */ let projectAlreadyHasViteTargets;
     ...
     * */ if (!validFoundTargetName.build && projectContainsUnsupportedExecutor) {
        ...
        if (alreadyHasNxViteTargets.build && (alreadyHasNxViteTargets.serve || !alreadyHasNxViteTargets.serve && projectType === 'library') && alreadyHasNxViteTargets.test) {
            throw new Error(`The project ${schema.project} is aready configured to use the @nrwl/vite executors.
    ...
    tasks.push(initTask);
    if (!(projectAlreadyHasViteTargets == null ? void 0 : projectAlreadyHasViteTargets.build)) {
        (0, _generatorUtils.addOrChangeBuildTarget)(tree, schema, buildTargetName);
    }
    if (!schema.includeLib && !(projectAlreadyHasViteTargets == null ? void 0 : projectAlreadyHasViteTargets.serve)) {
